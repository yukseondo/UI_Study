...
if ($wr_subject == '') {
    ...
    $error = true;
    $articleWrtieResult = array(
        'resultText' => "제목을 입력하세요."
    );
    echo json_encode($articleWrtieResult);
    return false;
}
...
if (substr_count($wr_content, '&#') > 50) {
    ...
    $error = true;
    $articleWrtieResult = array(
        'resultText' => "내용에 올바르지 않은 코드가 다수 포함되어 있습니다."
    );
    echo json_encode($articleWrtieResult);
    return false;
    exit;
...
if ($w == '' || $w == 'r') {
    ...
    //         $error = true;
    //         $articleWrtieResult = array(
    //             'resultText' => "너무 빠른 시간내에 게시물을 연속해서 올릴 수 없습니다."
    //         );
    //         echo json_encode($articleWrtieResult);
    //         return false;
    // }
